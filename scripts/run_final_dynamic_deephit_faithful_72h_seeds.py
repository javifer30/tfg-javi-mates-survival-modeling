...
from scripts.tune_dynamic_deephit_faithful_72h import build_run_config
from src.models.dynamic_72h.common import load_yaml, save_json, timestamp_utc
...
def run_final(config_path: str, dry_run=False, sample_size=None, device="auto", allow_collapsed=False):
    ...
    selection = json.loads((output_root / "best_hyperparameters.json").read_text(encoding="utf-8"))
    if selection.get("status") != "selected" and not allow_collapsed:
        raise ValueError("No accepted non-collapsed tuning selection. Review best_hyperparameters.json first.")
    selected = selection["selected"]
    params = json.loads(selected["hyperparameters"])
    config_id = selected["config_id"]
    logger = get_logger("run_final_dynamic_deephit_faithful_72h_seeds")
    planned, rows = [], []
    for seed in seeds:
        ...
        metrics = train_dynamic_deephit_faithful(config, logger)
        rows.append({
            ...
            "test_mean_tail_probability": metrics["splits"]["test"]["diagnostics"]["mean_tail_probability"],
    ...
    if rows:
        ...
        for name in ["test_ctd_antolini", "test_ibs", "test_ibll", "test_nbll", "test_mean_horizon_c_index", "test_std_risk10", "test_range_risk10", "test_mean_tail_probability"]:
...
def main():
    parser = argparse.ArgumentParser(description="Final three-seed Dynamic-DeepHit faithful 72h evaluation.")
