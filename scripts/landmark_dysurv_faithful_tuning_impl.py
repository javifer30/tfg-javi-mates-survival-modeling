...
from src.models.landmark_dynamic.common import expand_grid, load_yaml, save_json, save_yaml, timestamp_utc
...
def select_candidates(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "non_collapsed_alternative": sorted(non_collapsed, key=_sort_key)[0] if non_collapsed else None,
    }
...
def tune(config_path: str, dry_run=False, max_runs=None, sample_size=None, device="auto", force=False, resume=False):
    ...
    for index, raw_candidate in enumerate(candidates, start=1):
        ...
        try:
            metrics = train_dysurv_faithful(run_config, logger)
            rows.append(result_row(config_id, params, seed, run_dir, sample_size=sample_size, metrics=metrics))
    ...
    if combined_rows and not dry_run:
        ...
        if sample_size is not None:
            ...
            selection["sample_size"] = int(sample_size)
        save_json(output_root / "best_hyperparameters.json", selection)
    return planned


def main():
    parser = argparse.ArgumentParser(description="Validation-only tuning for DySurv faithful 72h.")
