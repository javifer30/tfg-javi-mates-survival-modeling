...
from scripts.tune_dysurv_static_faithful_72h import build_run_config
...
def run_final(config_path, dry_run=False, sample_size=None, device="auto", allow_collapsed=False):
    base = load_yaml(config_path)
    seeds = [int(seed) for seed in base["final"]["seeds"]]
    if seeds != FINAL_SEEDS:
        raise ValueError(f"Final static faithful seeds must be exactly {FINAL_SEEDS}")
    ...
    if selection.get("status") != "selected" and not allow_collapsed:
        raise ValueError("No accepted full-grid non-collapsed selection")
    selected = selection["selected"]
    params = json.loads(selected["hyperparameters"])
    config_id = selected["config_id"]
    logger = get_logger("run_final_dysurv_static_faithful_72h_seeds")
    planned, rows = [], []
    for seed in seeds:
        ...
        if dry_run:
            continue
        config = build_run_config(base, config_id, params, run_dir, seed, sample_size, device, True)
        metrics = train_dysurv_static_faithful(config, logger)
        rows.append({
...
def main():
    parser = argparse.ArgumentParser(description="Final three-seed static-only DySurv faithful evaluation.")
