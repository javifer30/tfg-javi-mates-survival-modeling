...
from scripts.landmark_static_tuning_impl import _model_list, _resolve_config_paths, _run_config, metric, save_config_snapshot, timestamp_utc
from src.models.landmark_static_pycox import train_landmark_static_model
...
def _validate_seeds(seeds):
    if [int(seed) for seed in seeds] != FINAL_SEEDS:
        raise ValueError(f"Final static_landmark seeds must be exactly {FINAL_SEEDS}")
...
def _write_comparison(final_root, prediction_time_hours):
    rows = []
    ...
    if rows:
        hours = int(float(prediction_time_hours))
        output_path = Path(final_root) / f"static_{hours}h_model_comparison.csv"
...
def run_final_models(config_path, requested_models=None, dry_run=False):
    logger = get_logger("run_final_static_landmark_seeds")
    tuning_config = load_yaml(config_path)
    seeds = tuning_config["final"].get("seeds", FINAL_SEEDS)
    _validate_seeds(seeds)
    output_root = Path(tuning_config["paths"].get("outputs_dir", "outputs/static_landmark"))
    ...
    for model_name in _model_list(tuning_config, requested_models):
        ...
        for seed in seeds:
            ...
            set_seed(int(seed))
            metrics = train_landmark_static_model(resolved, logger)
    ...
    if not dry_run:
        _write_comparison(final_root, tuning_config["experiment"]["prediction_time_hours"])
    return planned


def main():
    parser = argparse.ArgumentParser(description="Run final static_landmark_pycox models with exactly three seeds.")
