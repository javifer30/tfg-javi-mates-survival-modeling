...
def _model_list(config, requested):
    ...
    if unknown:
        raise ValueError(f"Unknown static_landmark model(s): {unknown}")
    return models
...
def _run_config(tuning_config, model_name, config_id, hyperparameters, output_dir, include_test, seed):
    run_config = {
        ...
        "paths": {
            ...
            "static_file_suffix": tuning_config.get("static_file_suffix", "static_landmark"),
        },
...
def tune_models(config_path, requested_models=None, dry_run=False, max_runs=None, resume=False):
    logger = get_logger("tune_static_landmark_models")
    tuning_config = load_yaml(config_path)
    seed = int(tuning_config["tuning"].get("seed", 42))
    output_root = Path(tuning_config["paths"].get("outputs_dir", "outputs/static_landmark")) / "tuning"
    planned = []
    run_count = 0
    ...
    for model_name in _model_list(tuning_config, requested_models):
        ...
        for index, hyperparameters in enumerate(expand_grid(tuning_config["models"][model_name].get("grid", {})), start=1):
            ...
            if resume and signature in completed:
                logger.info("Skipping completed static_landmark candidate: %s cfg %03d", model_name, index)
                continue
            ...
            try:
                ...
                logger.warning("static_landmark tuning candidate failed: %s %s: %s", model_name, config_id, exc)
...
def main():
    parser = argparse.ArgumentParser(description="Validation-only tuning for static_landmark_pycox models.")
