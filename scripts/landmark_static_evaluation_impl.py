...
def consolidate_final_static_landmark(config_path):
    logger = get_logger("evaluate_static_landmark_models")
    ...
    logger.info("static_landmark comparison saved to %s", output_path)


def main():
    parser = argparse.ArgumentParser(description="Consolidate static_landmark final model summaries.")
    parser.add_argument("--config", default="configs/landmark_static_evaluation.yaml")
    args = parser.parse_args()
    consolidate_final_static_landmark(args.config)
