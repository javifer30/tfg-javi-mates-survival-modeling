...
This module creates a benchmark cohort conditioned on patients still being
observable at the configured landmark time in ICU. Targets are measured relative
to that prediction time and administratively censored at 10 days.
"""
...
def load_landmark_static_base_table(config, logger):
    ...
    logger.info("static_landmark base table: %d stays after Y_i > %.1fh filter", len(df), pred_hours)
    return df
...
def make_landmark_static_split(df, config, logger):
    ...
    logger.info("static_landmark split sizes: train=%d validation=%d test=%d", len(train_df), len(val_df), len(test_df))
    return train_df.copy(), val_df.copy(), test_df.copy()


def build_landmark_static_preprocessor(config):
    prep_cfg = config["preprocessing"]
    return LandmarkStaticPreprocessor(
...
def validate_landmark_static_datasets(train_df, val_df, test_df, max_horizon_days=10.0):
    ...
    if ids["train"] & ids["validation"] or ids["train"] & ids["test"] or ids["validation"] & ids["test"]:
        raise ValueError("patientunitstayid overlap detected between static_landmark splits")
...
def build_summary(train_df, val_df, test_df, preprocessor, config):
    pred_hours = float(config["target"]["prediction_time_hours"])
    summary = {
        "pipeline": config.get("experiment", {}).get("name", "static_landmark_pycox"),
        "methodology": {
...
def build_landmark_static_dataset(config, logger):
    ...
    validate_landmark_static_datasets(train, val, test, config["target"]["max_horizon_days"])

    paths = config["paths"]
    output_dir = Path(paths["processed_dir"])
    output_dir.mkdir(parents=True, exist_ok=True)
    suffix = config.get("output_file_suffix", "static_landmark")
    ...
    logger.info("static_landmark datasets saved in %s", output_dir)
