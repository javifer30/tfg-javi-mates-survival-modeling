...
from scripts.tune_static_72h_models import select_best_row
from src.data.static_72h_dataset import (
    DURATION_COL,
    EVENT_COL,
    ID_COL,
    RAW_EVENT_COL,
    REL_DURATION_DAYS_COL,
    Static72hPreprocessor,
    load_static_72h_base_table,
    validate_static_72h_datasets,
)
from src.evaluation.static_72h_metrics import eval_surv_metrics, horizon_c_index
...
def test_static_72h_target_filters_and_caps(tmp_path):
    flat = pd.DataFrame(
        {
    ...
    df = load_static_72h_base_table(config, _logger())
...
def test_static_72h_preprocessor_train_fit_and_validation_checks():
    raw = pd.DataFrame(
        {
            ID_COL: [1, 2, 3],
    ...
    validate_static_72h_datasets(train, val, test, max_horizon_days=10)
...
def test_static_72h_tuning_selection_ignores_failed_rows():
    rows = [
