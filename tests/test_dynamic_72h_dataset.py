...
def test_dynamic_72h_build_uses_static_ids_and_strict_72h_filter(tmp_path):
    ...
    arrays, summary = build_dynamic_72h_dataset(_config(tmp_path), _Logger(), dry_run=True)
    ...
    assert summary["checks"]["ids_match_static_72h_exact_order"] is True
...
def test_dynamic_72h_write_outputs(tmp_path):
    train = _static_df([101, 102], "train")
    val = _static_df([201], "validation")
    test = _static_df([301], "test")
    train.to_parquet(tmp_path / "train.parquet", index=False)
    ...
    build_dynamic_72h_dataset(_config(tmp_path), _Logger(), force=True, dry_run=False)

    assert (tmp_path / "dynamic" / "train_dynamic_72h.npz").exists()
    assert (tmp_path / "dynamic" / "val_dynamic_72h.npz").exists()
    assert (tmp_path / "dynamic" / "test_dynamic_72h.npz").exists()
    assert (tmp_path / "audit" / "dynamic_72h_data_audit.json").exists()
    loaded = np.load(tmp_path / "dynamic" / "train_dynamic_72h.npz")
